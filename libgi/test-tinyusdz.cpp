...
void CreateScene(tinyusdz::Stage *stage) {
  ...
  tinyusdz::GeomMesh mesh;
  {
...

	{
		...
		mesh.creaseIndices.set_value(creaseIndices);

		std::vector<float> creaseSharpness;
		creaseSharpness.push_back(7.5f);
		mesh.creaseSharpnesses.set_value(creaseSharpness);
	}
...
void testusd2(std::string filepath) {
	std::cout << std::endl << "USD test 2:" << std::endl << std::endl;
	...
	tinyusdz::Path path = tinyusdz::Path::make_root_path();
	//path.append_prim("root");
	//path.append_prim("Cube");
	path.append_prim("root");
	path.append_prim("TantoKnife_LowPoly_002");
	nonstd::expected<const tinyusdz::Prim*, std::string> exp_cube_form = stage.GetPrimAtPath(path);
	const tinyusdz::Xform *cube_form = exp_cube_form.value()->as<tinyusdz::Xform>();
	std::cout << "Name: " << cube_form->get_name() << std::endl;

	//path.append_prim("Cube_001");
	path.append_prim("Mesh_004");
	nonstd::expected<const tinyusdz::Prim*, std::string> exp_mesh = stage.GetPrimAtPath(path);
	const tinyusdz::GeomMesh *mesh = exp_mesh.value()->as<tinyusdz::GeomMesh>();
	std::cout << "Face vertex count: " << mesh->get_faceVertexCounts() << std::endl;
	...
	//tinyusdz::Xform cube_form = stage.GetPrimAtPath(path);
	//tinyusdz::GeomMesh mesh = stage.GetPrimAtPath(path);
}
