		...
		platform::platform(const std::vector<std::string> &args) : wf::platform("opengl") {
			...
			if (texture_support_mode == PROPER_BINDLESS)
				if (!GLEW_ARB_bindless_texture) {
					...
					std::cerr << "Your GPU only supports bindless textures with dynamically uniform access pattern, which our shaders do not produce, so textured materials will rely on a hacky implementation with probably lower performance" << std::endl;
