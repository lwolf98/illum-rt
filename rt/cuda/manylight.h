#pragma once

#include "platform.h"

#include "base.h"
#include "rng.h"

//#include "gi/direct-steps.h"
#include "gi/manylight-steps.h"
#include "gi/manylight.h"

#include <curand.h>

/* 
 * This file contains wf steps for the manylight algorithm
 *
 */

namespace wf::cuda {

	/*struct sample_v_0s : public wf::wire::sample_v_0s<raydata, float4> {
		random_number_generator<float2> rng;
		void run() override;
	};*/

	/*struct create_vpls : public wf::wire::create_vpls<raydata, float4, vpl> {
		void run() override;
	};

	struct russian_roulette : public wf::wire::russian_roulette<raydata, float4> {
		void run() override;
	};

	struct sample_next_vpls : public wf::wire::sample_next_vpls<raydata, float4, vpl> {
		void run() override;
	};*/

	/*struct copy_vpls : public wf::wire::copy_vpls<vpl, std::vector<vpl>> {
		void run() override;
	};

	struct sample_vpls : public wf::wire::sample_vpls<raydata, std::vector<vpl>, vpl> {
		void run() override;
	};*/

	struct integrate_vpl_samples : public wf::wire::integrate_vpl_samples<raydata, vpl> {
		void run() override;
	};
	
}