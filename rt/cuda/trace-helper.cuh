...
__forceinline__ __device__ uint32_t len(wf::cuda::subd_patch &patch) {
	return std::pow(2, patch.subd_level)+1;
}

__forceinline__ __device__ int32_t get_subd_quad(wf::cuda::subd_patch &patch, int quad_ref) {
	int x = quad_ref % len(patch);
	int y = quad_ref / len(patch);
	return y * len(patch) + x;
}

__forceinline__ __device__ triangle subd_tri(wf::cuda::subd_patch &patch, int quad_ref, bool upper) {
	triangle tri;

	//TODO:
	// get subd quad by morton code rather than the currently used position code?
	int quad_id = get_subd_quad(patch, quad_ref);
	tri.material_id = patch.material_id;
	if (upper) {
		tri.a = patch.start_index + quad_id;
		tri.b = patch.start_index + quad_id + len(patch); // vert down
		tri.c = patch.start_index + quad_id + 1; // vert right
	}
	else {
		tri.a = patch.start_index + quad_id + len(patch); // vert down
		tri.b = patch.start_index + quad_id + len(patch) + 1; // vert down right
		tri.c = patch.start_index + quad_id + 1; // vert right
	}
