	...
	namespace cuda {
		...
		void batch_cam_ray_setup::run() {
			...
			if(rt->use_incoherence){
				...
				setupRayIncoherent<<<blocks, threads>>>(resolution,
				...
				setupRay<<<NUM_BLOCKS_FOR_RESOLUTION(resolution), DESIRED_BLOCK_SIZE>>>(U, V, cam.near_w, cam.near_h, resolution,
