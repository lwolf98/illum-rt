...
namespace wf::cuda {
	...
	//REVIEW: actually hit_ns, right? else: use diff_geom ns/ng
	static __device__ float3 hit_ng(const tri_is &hit, const uint4 &tri, const float4 *vert_norm) {
	...
	namespace k {
		static __global__ void sample_v_0s(int2 res, float4 *light_rays,
										   float3 *light_throughput, float3 *last_vpls_pos, float3 *le, int *vpl_store_offset,
	...
	void sample_v_0s::run() {
		...
		k::sample_v_0s<<<launch_config>>>(res,
										  ...
										  last_vpls_pos->data.device_memory,
	...
	namespace k {
		static __global__ void sample_next_vpls(int2 res, float4 *light_rays, tri_is *hits,
										   ...
										   int *vpl_store_offset, float3 *last_vpls_pos, int depth,
			...
			float3 last_vpl_pos = last_vpls_pos[ray_index]; //TODO!!!
			...
			diff_geom vpl_dg(vpl_is, last_vpl_pos, f3(vpl_w_in), refs);
			//diff_geom_ml vpl_dg(vpl_is, refs);
			if (vpl_is.valid()) {
			...
			light_throughput[ray_index] = throughput;
			last_vpls_pos[ray_index] = f3(vpl_pos);
		}
	}
	...
	namespace k {
		static __global__ void integrate_vpl_samples(int2 res,
			...
			float3 camray_org = f3(camrays[ray_index*2+0]);
			float3 camray_dir = f3(camrays[ray_index*2+1]);
			float3 vpl_pos = f3(vpls_pos[vpl_indices[ray_index]]);
			...
			if (hit.valid() && !shadow_hit.valid() && vpl_is.valid()) {
				diff_geom vpl_dg(vpl_is, vpl_pos, f3(shadowray_dir), refs);
				diff_geom x_dg(hit, camray_org, camray_dir, refs);
				/*if (x < 20)
				...
				float3 from = camray_org + hit.t * -x_w_o;
				float t = length(vpl_pos - from);
