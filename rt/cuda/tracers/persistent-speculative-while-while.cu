#include "persistent-speculative-while-while.h"
...
namespace wf{
	namespace cuda{
		void persistentspeculativewhilewhile::compute_hit(bool anyhit) {
			...
			persistentspeculativewhilewhileTrace<<<DESIRED_BLOCKS_COUNT, DESIRED_BLOCK_SIZE>>>(rc->resolution().x * rc->resolution().y,
