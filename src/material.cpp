...
glm::vec3 layered_brdf::f(const diff_geom &geom, const glm::vec3 &w_o, const glm::vec3 &w_i) {
...
glm::vec3 lambertian_reflection::f(const diff_geom &geom, const glm::vec3 &w_o, const glm::vec3 &w_i) {
	if (!same_hemisphere(w_i, geom.ns)) return vec3(0);
	return one_over_pi * geom.albedo();
}

glm::vec3 phong_specular_reflection::f(const diff_geom &geom, const glm::vec3 &w_o, const glm::vec3 &w_i) {
	if (!same_hemisphere(w_i, geom.ns)) return vec3(0);
