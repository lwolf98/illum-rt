...
namespace wf
{
	...
	public:
		...
		virtual void use(raydata *camdata, raydata *bouncedata, per_sample_data<float> *pdf_light, per_sample_data<float> *pdf_other, compute_light_distribution *light_dist, per_sample_data<vec3> *light_col) = 0;
	};
	namespace wire
	{
		...
		public:
			...
			bool properly_wired()
			{
				return camdata && bouncedata && pdf_light && pdf_other && light_dist && light_col;
			}

			void use(raydata *camdata, raydata *bouncedata, per_sample_data<float> *pdf_light, per_sample_data<float> *pdf_other, compute_light_distribution *light_dist, per_sample_data<vec3> *light_col)
			{
				this->camdata = dynamic_cast<RD *>(camdata);
				this->bouncedata = dynamic_cast<RD *>(bouncedata);
