		...
		for (int i = 0; i < paths; i++) {
			...
			for (int j = 1; j <= path_length; ++j) {
				...
				if(spec_surface != nullptr) { 
					...
					auto [w_o, f, pdf, interact_type] = hit.mat->brdf->sample(v_j_geometry, -v_j.w_in, rc->rng.uniform_float2()); //f(v_j-1->v_j->v_j+1)
...
vec3 manylight_algorithm::sample_pixel(uint32_t x, uint32_t y) {
	...
	if (vpls.size() != 0) {
		int i;
		for (i = vpl_index; i < vpl_index+vpls_per_sample; ++i) {
			//if (i == cmp_index && cmp_index != 0)
			//	i++;
			if (i >= vpls.size())
				break;
			vpl v = vpls[i];
			...
			if (!rc->scene.rt->any_hit(shadow_ray)) {
				...
				if (x == 44 && y == 55 && debugging)
					cout << "VPL[" << i << "," << v.is.ref << "]: " << f_x*G*v.col*f_v << endl;
				...
				if (x == 44 && y == 55 && debugging)
						cout << "VPL[" << i << "," << v.is.ref << "]: no valid hit or in shadow!" << endl;
			}
			//if (x == 0 && y == 0)
			//	cout << i << " s: " << current_sample_index << endl;
		}
		next_index[y * rc->resolution().x + x] = i;
...
namespace wf {
	...
	void manylight_algorithm::regenerate_steps() {
		...
		*/
		...
		last_vpls_pos = rc->platform->allocate_vec3_per_sample_manually(paths);
		...
		if (!rc->ml_cpu_preparation) {
			...
			sample_v_0->use(vpl_rays, light_throughput, last_vpls_pos, le, vpl_store_offset, l_dist);
			...
			for (int depth = 0; depth < path_length; ++depth) {
				...
				sample_next_vpl->use(vpl_rays, light_throughput, last_vpls_pos, vpl_store, vpl_store_offset, depth);
